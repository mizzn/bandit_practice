...
class Environment:
    def __init__(self):
        self.probs = [0.3, 0.5, 0.7] #各腕の真の報酬確率
...
class Agent:
    ...
    def __init__(self, eps, correct_probs, policy):
        self.epsilon = eps
        self.arm_size = 3
...
class Manegement:
    ...
    def start(self):
        for i in range(self.sim):
            self.agent = Agent(self.eps, self.correct_probs, self.policy)

            for i in range(self.step):
                # エージェントによって腕(インデックス)を選択する
                selected_arm = self.agent.policy()
...
if __name__ == "__main__":
    eps = 0.1
    print("policy = "+str(), end="")
    policy = input()

    management = Manegement(eps, policy)
    management.start()
